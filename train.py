...
def train(
    ...
    with mlflow.start_run():
        ...
        if search=='KFold':
            ...
            scores = cross_validate(model, features_train, target_train, cv = cv, scoring=('accuracy', 'f1_macro','roc_auc_ovr'))
            ...
            scores = cross_validate(search_q, features_train, target_train, cv = cv_outer, scoring=('accuracy', 'f1_macro','roc_auc_ovr'))
            accuracy=scores['test_accuracy'].mean()
            f1_macro =scores['test_f1_macro'].mean()
            roc_auc_ovr =scores['test_roc_auc_ovr'].mean()
            
            search_q.fit(features_train,target_train)
            print('The parameters combination that would give best accuracy is : ',search_q.best_params_)
            if(select_model=='logist_regression'):
